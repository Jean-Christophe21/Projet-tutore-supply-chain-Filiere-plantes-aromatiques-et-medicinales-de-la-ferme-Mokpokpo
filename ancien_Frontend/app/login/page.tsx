...
function LoginFormContent() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // CORRECTION ICI : Gestion améliorée des types d'erreurs
      console.error("Erreur de login:", err) // Utile pour le debug (F12)

      let errorMessage = "Une erreur inconnue est survenue."

      if (typeof err?.detail === "string") {
        // Erreur renvoyée par l'API (ex: "Identifiants incorrects")
        errorMessage = err.detail
      } else if (err?.message) {
        // Erreur réseau ou JavaScript (ex: "Failed to fetch")
        errorMessage = `Erreur technique : ${err.message}`
      } else if (typeof err === "string") {
        errorMessage = err
      }

      // Si c'est une erreur réseau (serveur éteint), on met un message plus clair
